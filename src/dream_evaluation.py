...
from model.dream_klass import bind_local_dream_generation
    ...
    for example_idx, example in tqdm(enumerate(data), total=len(data), desc=f"Generating completions for {dataset.capitalize()}"):
        ...
        for sample_idx in range(num_samples):
            output = model.diffusion_generate(
                ...
                tilg_extra_kl_threshold=tilg_extra_kl_threshold,
            )
...
def test_humaneval(
    ...
    if alg == "klass":
        ...
        save_dir = (
            f"{save_dir}/Dream/humaneval/{alg}/{unmask_strategy}/len_{gen_length}/steps_{steps}/"
            f"conf{conf_threshold}_floor{tilg_extra_conf_floor}_extra{tilg_extra_max}_ratio{tilg_extra_ratio}_"
            f"empty{tilg_extra_allow_empty_base}_ema{tilg_ema_decay}_gw{guidance_weight}_l{tilg_rerank_lambda}_"
            f"streak{history_gate_min_streak}_escape{history_gate_confidence_escape}_s{num_samples}"
        )
    else:
    ...
    for task_id, info in tqdm(data.items(), desc="Generating completions for HumanEval"):
        ...
        for sample_idx in range(num_samples):
            ...
            used_steps = getattr(output, "used_steps", steps)
...
def main():
    ...
    parser.add_argument('--alg', type=str, choices=['klass', 'maskgit_plus', 'origin', 'entropy', 'klass_ours_light', 'klass_ours_full', 'klass_tilg_history_extra', 'klass_tilg_history_kl_extra', 'klass_tilg_extra_only', 'klass_tilg_extra_kl', 'confidence_threshold_tilg_history_gate_capped_extra', 'dream_tilg_assist', 'dream_klass_tilg'], default='maskgit_plus', help='Diffusion algorithm to use')
    parser.add_argument('--unmask_strategy', type=str, choices=['all', 'max_conf', 'min_kl', 'random'], default='all', help='Unmasking strategy')
    parser.add_argument('--guidance_weight', type=float, default=0.3, help='TILG guidance weight')
    parser.add_argument('--tilg_ema_decay', type=float, default=0.95, help='TILG EMA decay')
    parser.add_argument('--tilg_rerank_lambda', type=float, default=0.05, help='TILG rerank lambda')
    parser.add_argument('--tilg_extra_conf_floor', type=float, default=0.75, help='TILG extra confidence floor')
    parser.add_argument('--tilg_extra_ratio', type=float, default=0.25, help='TILG extra ratio')
    parser.add_argument('--tilg_extra_max', type=int, default=2, help='TILG max extra tokens per step')
    parser.add_argument('--tilg_extra_allow_empty_base', type=int, default=1, help='Allow extra decode when base ready is empty')
    parser.add_argument('--history_gate_min_streak', type=int, default=3, help='Min top1 streak for history gate')
    parser.add_argument('--history_gate_confidence_escape', type=float, default=0.95, help='Confidence escape threshold')
    parser.add_argument('--tilg_extra_kl_threshold', type=float, default=0.01, help='Extra KL threshold for TILG variants')
    ...
    bind_local_dream_generation(model, alg=args.alg)
    print("[DEBUG] diffusion_generate:", model.diffusion_generate)
    print("[DEBUG] sample_impl:", model._sample)
    ...
    if args.dataset == 'humaneval':
        ...
        test_humaneval(
            ...
            tilg_extra_kl_threshold=args.tilg_extra_kl_threshold,
        )
